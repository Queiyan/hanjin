...
{
    public partial class KeyboardMsgWindow : Form
    {
        private CustomVirtualKeyboard _keyboard;
        public static bool IsShowing { get; private set; } = false;

        // 60초 뒤 홈 화면으로 이동하는 타이머
        private System.Timers.Timer inactivityTimer;

        public KeyboardMsgWindow(string msg, CustomVirtualKeyboard keyboard)
        {
            if (IsShowing)
            {
            ...
            //this.FormClosed += (s, e) =>
            //{
            //    IsShowing = false; // 창 닫힐 때 상태 초기화
            //    keyboard.EnableKeyboard(true); // 키보드 활성화
            //};

            this.TopMost = true; // 최상단에 위치하도록 설정

            _keyboard = keyboard;
            //_keyboard.EnableKeyboard(false); // 키보드 비활성화
